...
export function GraphPanel({ graph }: { graph: RunResult["graph"] }) {
  ...
  return (
    <SectionCard title="Task lineage graph" subtitle="Use this to explain how task -> spec -> design -> review -> lesson evolved.">
      {graph.nodes.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-slate-200 p-8 text-sm text-slate-500">
          Graph data will appear after a run or after calling /api/graph/[taskId].
        </div>
      ) : (
